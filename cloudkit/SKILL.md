...
### Multi-device writer roles

When one device is the primary writer and others are readers:

- Bootstrap **secondary** devices **receive-only** (`fetchChanges` only). Do not
  auto-`sendChanges` / push a local mirror on launch — a stale secondary App
  Group or cache will fight the primary's revisions.
- On the **primary**, do **not** mirror remote `fetchedRecordZoneChanges`
  modifications onto the local store of record when that mirror can overwrite a
  fresher local upsert. Keep server `CKRecord`s for change tags; apply remote
  **deletions** if the product requires them. Secondaries mirror modifications
  into their local store.
- Before save, reuse a cached/fetched `CKRecord` (correct change tag) rather
  than inserting a bare `CKRecord(recordID:)`. Otherwise CloudKit returns
  `CKError` 14 (record to insert already exists). On `.serverRecordChanged`,
  keep `serverRecord` and reschedule.

```swift
// Prefer cached server record so the change tag survives.
let record = serverRecordsByID[recordID]
...
| Secondary auto-pushes on launch | Receive-only bootstrap (`fetchChanges`); push only on explicit publish. |
| Primary mirrors remote mods over local | Keep change-tag cache; do not clobber fresher local upserts. |
| Saving a new `CKRecord` without a fetch | Reuse server record / change tag or hit insert-exists (`CKError` 14). |
| `CKQuery` over a new/unindexed type | Prefer `recordZoneChanges(inZoneWith:since:)` when listing a private zone without Dashboard indexes — `CKQuery` often fails with “Field 'recordName' is not marked queryable” (`CKError` 12 / internal 2015). |
| Widget / appex needs full feed fast | Also publish a fixed-name snapshot record (`record(for:)` by ID, JSON/bytes payload). Zone-scan alone often loses to WidgetKit budgets; push wake + snapshot read is the reliable cross-device Home Screen path. |
| Pushing widgets the instant save returns | `save` returning ≠ confirmed durable state. Read-after-write (hash match) and re-check the local source before `apns-push-type: widgets`; do not paper over with fixed sleeps. |
