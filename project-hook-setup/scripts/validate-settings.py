...
def validate():
    ...
    for hook_type, config in REQUIRED_HOOKS.items():
        ...
        # Check matcher (or no matcher for SessionStart/UserPromptSubmit)
        found_entry = False
        for entry in hook_entries:
            entry_matcher = entry.get("matcher")
            if config["matcher"] is None or entry_matcher == config["matcher"]:
                found_entry = True
                # Check scripts
        ...
        if not found_entry:
            if config["matcher"]:
                ...
                errors.append(f"Missing {hook_type} configuration")
                print(f"  ❌ No {hook_type} entry found")
