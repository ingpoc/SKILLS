...
**Before API testing**, ensure servers are running cleanly:

```bash
# Clean restart all servers (clears cache, ports)
.claude/scripts/restart-servers.sh

# Verify health endpoint
curl -s http://localhost:3001/health | grep "healthy" || echo "API not ready"
```
...
|--------|---------|----------|
| `restart-servers.sh` | Clean restart all servers | `.claude/scripts/` |
| `run-tests.sh` | Unit + API tests (includes health check) | `.claude/scripts/` |
| `health-check.sh` | Verify infrastructure + servers | `.claude/scripts/` |
...
```bash
# 1. Buyer places order via API
curl -s -X POST http://localhost:3001/api/checkout \
  -H "Content-Type: application/json" \
  -d '{"sessionId":"test-integration"}' | jq .

# 2. Seller receives order (query seller API)
curl -s http://localhost:3002/api/orders | jq '.[] | select(id=="test-integration")'

# 3. Seller updates status
ORDER_ID="test-integration"
curl -s -X PUT "http://localhost:3002/api/orders/$ORDER_ID" \
  -H "Content-Type: application/json" \
  -d '{"status":"processing"}' | jq .

# 4. Buyer sees status change
curl -s "http://localhost:3000/api/orders/$ORDER_ID" | jq '.status'
```

### Cross-Origin Verification

```bash
# Verify CORS headers from buyer to API
curl -s -I -X OPTIONS http://localhost:3001/api/search \
  -H "Origin: http://localhost:3000" \
  -H "Access-Control-Request-Method: GET" | grep -i "access-control"

# Verify CORS headers from seller to API
curl -s -I -X OPTIONS http://localhost:3001/api/orders \
