...
**Test ONE app completely before moving to the next**:

### Phase 1: Buyer App (localhost:3000)

| Test | Action | Verify |
|------|--------|--------|
| Search | Navigate, enter search query | Results display correctly |
| Product Details | Click product | Details page loads, images display |
| Add to Cart | Click "Add to cart" | Cart counter updates |
| Cart Page | Navigate to cart | Items show, total calculates |
| Checkout | Fill form, submit | Order created, success message |
| Orders Page | Navigate to orders | Order list shows new order |
| Order Detail | Click order | Status, items display correctly |

### Phase 2: Seller App (localhost:3002)

| Test | Action | Verify |
|------|--------|--------|
| Catalog | Navigate to catalog | Products list correctly |
| Add Product | Fill form, submit | New product appears in list |
| Orders | Navigate to orders | All orders display |
| Order Detail | Click order | Full order details visible |
| Update Status | Change status | Status updates, reflects to buyer |
| Configuration | Navigate to config | Settings load and save |

### Phase 3: Integration (Cross-App Flow)

| Test | Action | Verify |
|------|--------|--------|
| Order Sync | Buyer places order → Seller | Order appears in seller orders |
| Status Sync | Seller updates status → Buyer | Buyer sees new status |
| Cancellation | Buyer cancels → Seller | Seller sees cancellation |
| Real-time | Update in one app | Other app refreshes/updates |

**Testing tool**: Load `browser-testing` skill

---

## Context Graph Usage

**Query context graph** (when tests fail):

```bash
# Before fixing browser issues, check for similar problems
context_query_traces("browser navigation error")
context_get_trace(trace_id)  # Read full context
```

**Store success** (AFTER verified working):

```bash
# Only store AFTER fix is tested and working
context_store_trace({
  decision: "Fixed [specific browser issue] with working solution",
  category: "error",
  outcome: "success",
  feature_id: "SDK-BUYER-ORDERS-003"
})
```

**Never store unverified fixes.**
