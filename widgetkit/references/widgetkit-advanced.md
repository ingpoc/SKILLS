...
**Auth Key JWT:** reuse one provider token for ~50 minutes (Apple allows up to
60). A new JWT on every send → `TooManyProviderTokenUpdates`. **Acceptance:**
APNs HTTP 200 is not Home Screen proof — the provider must update App Group /
refetch before rendering; compare live widget capture to the writer’s feed.
